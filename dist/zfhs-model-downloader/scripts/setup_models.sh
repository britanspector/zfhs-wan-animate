#!/usr/bin/env bash
# Symlink P07 models from autodl-fs local storage into {COMFYUI_ROOT}/models/
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
COMFY_ROOT="${COMFYUI_ROOT:-/root/ComfyUI}"
MODELS_STORE="${ZFHS_MODELS_STORE:-$(if [[ -d /autodl-fs/data ]]; then echo /autodl-fs/data; elif [[ -d /autodl-fs ]]; then echo /autodl-fs; else echo /autodl-fs/data; fi)/zfhs-wan-animate/models}"
MANIFEST="${ROOT}/manifest/models.yaml"
STRICT=false
...
Symlink P07 model files from autodl-fs local storage into ComfyUI models directory.
...
if [[ -n "${ZFHS_MODEL_SOURCES:-}" ]]; then
  log_warn "ZFHS_MODEL_SOURCES is deprecated and ignored; using models.yaml + ZFHS_MODELS_STORE"
fi
...
mapfile -t ENTRIES < <(python3 - "$MANIFEST" "$MODELS_STORE" <<'PY'
import sys
import yaml
...
models_store = sys.argv[2].rstrip("/")

for item in data.get("models", []):
    category = item.get("category", "")
    rel = item.get("path", "")
    if category and rel:
        source = f"{models_store}/{category}/{rel}"
...
log_info "Models store: ${MODELS_STORE}"
...
if $STRICT && [[ $missing -gt 0 ]]; then
  log_error "Strict mode: $missing model source(s) missing from autodl-fs store"
  log_error "Run: bash scripts/download_models.sh"
  exit 1
fi
