...
# Align ONNX Runtime CUDA/cuDNN with PyTorch bundled libraries.
...
def _ensure_cudnn_library() -> None:
    import ctypes
    import glob
    import os
    import site
    ...
    for sp in site.getsitepackages():
        for lib in glob.glob(os.path.join(sp, "nvidia", "cudnn", "lib", "libcudnn.so*")):
            if os.path.basename(lib).endswith(".9") or "libcudnn.so.9" in lib:
                try:
...
_ensure_cudnn_library()


def _build_providers(device: str) -> tuple[list[str], list[dict] | None]:
...
class SimpleOnnxInference(object):
    def __init__(self, checkpoint, device='CUDAExecutionProvider', **kwargs):
        ...
        self.device = device
        self.providers, self.provider_options = _build_providers(device)
        self.session = None
        self._open_session()

    def _open_session(self):
        session_kwargs: dict = {"providers": self.providers}
        if self.provider_options is not None:
            session_kwargs["provider_options"] = self.provider_options
        self.session = onnxruntime.InferenceSession(self.checkpoint, **session_kwargs)
    ...
    def reinit(self, provider=None):
        # Always recreate the session. A failed CUDA run can leave a broken session
        # behind while session is still non-None, which then poisons cached models.
        if provider is not None:
            self.device = provider
            self.providers, self.provider_options = _build_providers(provider)
        self.cleanup()
        self._open_session()
