...
> **通用迁移方法论**（从 zealman-app 剥离单功能模块）：个人 Skill `~/.cursor/skills/zealman-module-extract/`

## 独立部署（AutoDL，无 zealman-app）
...
bash scripts/start-wan-animate.sh --autodl    # ComfyUI + Web + Jupyter + 公网网关
```

配置开机自启：`bash scripts/setup-autodl-autostart.sh`

详见 [docs/STANDALONE.md](docs/STANDALONE.md) 与 [docs/AUTODL.md](docs/AUTODL.md)。

## 一键启动（推荐）

```bash
cd /root/zfhs-wan-animate
bash scripts/start-wan-animate.sh --autodl          # AutoDL 三服务（公网 6006/6008）
bash scripts/start-wan-animate.sh --with-comfy      # 本地开发：ComfyUI + Web (6020)
...
| 预设 | 文件 | 说明 |
|------|------|------|
| **v4** 标准动作迁移 | `workflows/p07_animate_v4.json` | API Prompt；画布用 `assets/workflows/ui/p07_animate_v4_ui.json` |
| **v5** 保身份动作迁移 | `workflows/p07_animate_v5.json` | API Prompt；画布用 `assets/workflows/ui/p07_animate_v5_ui.json` |
...
| 端口 | 服务 | 用途 |
|------|------|------|
| **6006** | ComfyUI 原生 Web UI | **节点画布**、Load 工作流、Queue Prompt；AutoDL 公网 `u...` |
| **6008** | nginx 网关 | AutoDL 公网 `uu...`：Web `/` + Jupyter `/jupyter/` |
| **6020** | wan-animate-api + 莫兰迪前端 | 内部端口，经 6008 对外 |
| **8888** | Jupyter Lab | 内部端口，经 6008 `/jupyter/` 对外 |
| 5173 | Vite dev | 开发前端，代理到 6020 |

- 代码 / CLI / Notebook 均通过 HTTP 直连 ComfyUI **6006**
- **画布 Load**（UI 格式，含 `nodes`/`links`）：
  - v4：`assets/workflows/ui/p07_animate_v4_ui.json`
  - v5：`assets/workflows/ui/p07_animate_v5_ui.json`
- **API Prompt 格式**（仅 `/prompt`、Web、Notebook，**勿在画布 Load**）：
  - `workflows/p07_animate_v4.json`、`workflows/p07_animate_v5.json`
- **URL 自动加载 P07 v4**（运行 `bash scripts/setup_comfy_p07_template.sh` 并重启 ComfyUI 后）：`http://127.0.0.1:6006/?template=p07_wan22_animate_v4&source=zfhs-workflow-templates`

## Notebook 分步运行

在 Jupyter 中打开 [`notebooks/中升智学动作迁移实验项目教学代码.ipynb`](notebooks/中升智学动作迁移实验项目教学代码.ipynb)，从上到下依次执行，从配置 → 验资产 → 上传 → 提交 → 轮询 → 预览，默认 **5 秒**试跑。

```bash
# 确保 ComfyUI 已在 6006 运行
bash scripts/start-comfyui.sh
# 或一键：bash scripts/start-wan-animate.sh --with-comfy
```
...
| 1003 | 帧数 | seconds×30（不超过参考视频时长） |
| 867 | 输出 | `output/zfhs-wan-animate/角色迁移_{图名}_{视频名}` |
